...
export function Select({
  ...
  return (
    <div className={`select__container ${color} `}>
      <label htmlFor={`id-${name}`}>{label}</label>
      <div ref={ref}>
        <button
          className={`select__selected ${selectOpen ? 'open' : ''}`}
        ...
        {selectOpen ? (
          <div className={`select ${multiSelect ? 'multiselect' : ''}`}>
            {multiSelect ? (
...
export function FilterSelect({ children, clean = 'x', filter = 'f', action = () => null, title = 'Filtrar' }) {
